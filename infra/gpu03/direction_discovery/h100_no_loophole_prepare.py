"""Versioned CPU-only H100 token/Q preparation for the matched no-hint audit.
...
from infra.gpu03.direction_discovery import h100_no_loophole_protocol as plan
...
MODULE = "infra.gpu03.direction_discovery.h100_no_loophole_prepare"
HERE = "infra/gpu03/direction_discovery/h100_no_loophole_prepare.py"
PLANNER = "infra/gpu03/direction_discovery/h100_no_loophole_protocol.py"
...
def check_sources(spec):
    ...
    plan.check_frozen_source()
    sources = spec["source_files"]
    require(isinstance(sources, dict) and {HERE, PLANNER, plan.FROZEN_HERE, *plan.SOURCES} <= set(sources), "Incomplete preparation source bindings")
    actual = set()
...
def context(spec_ref):
    ...
    require(spec.get("schema_version") == 1 and spec.get("purpose") == "prepare_h100_no_loophole_cpu_inputs" and
    ...
    plan.validate_portability(spec["h100_portability"], old["conditions"])
    require(spec["h100_portability"]["runtime_replacement"]["python_binary"] == spec["python_binary"],
            "Preparation Python differs from qualified portability metadata")
...
def projection_audit(ctx, data, hashes):
    ...
    return {"status": "passed", "torch_version": VERSIONS["torch"], "source_sha256": plan.SOURCES,
            ...
            "h100_portability_sha256": plan.digest(ctx["spec"]["h100_portability"]),
            "physical_conditions": {k: v for k, v in plan.translate_conditions(ctx["old"]["conditions"], ctx["spec"]["h100_portability"]).items() if v["layers"]}}
...
def verify_projections(spec_ref, directory):
    ...
    reconstructed, hashes = projection_snapshot(plan.translate_conditions(ctx["old"]["conditions"], ctx["spec"]["h100_portability"]))
...
def prepare(spec_ref):
    ...
    vectors, hashes = projection_snapshot(plan.translate_conditions(ctx['old']['conditions'], ctx['spec']['h100_portability']))
    ...
    bundle = {'schema_version': 1, 'purpose': 'canonical_no_loophole_prompt_package', 'status': 'tokenized_verified',
              ...
              'h100_portability': ctx['spec']['h100_portability']}
...
def verify(spec_ref, bundle_ref, producer_exit_ref, output_proof):
    ...
    require(bundle.get('preparation_spec') == spec_ref and bundle.get('preparation_source_files') == ctx['spec']['source_files'] and
            bundle.get('h100_portability') == ctx['spec']['h100_portability'], "Package source/spec/portability differs")
