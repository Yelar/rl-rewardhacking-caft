...
def main():
 ...
 for row,(metric,label) in enumerate(metrics):
  ...
  for col,setting in enumerate(['fixed','randomized']):
   ...
   for x,(cap,mode) in enumerate(configurations):
    ...
    suffix="†" if (cap,mode,setting,metric)==(3072,"off","randomized","evaluator_presence") else ""
    ax.annotate(f"{r['successes']}/1190{suffix}",(x,hi*100),xytext=(0,6),textcoords='offset points',ha='center',fontsize=8,color=color)
 ...
 fig.text(.09,.025,'Three completed 3,072-token conditions; ON randomized pending (40 partial rows excluded).\n119 problems × 10 samples per cell; same checkpoint, prompts and seed policy.\nBars: 95% whole-problem bootstrap intervals. Native classifier shown; helper unknown bounds reported separately.\nZero-event intervals collapse, not population upper bounds. ON includes materialization/rounding. Cross-budget pairing is exploratory.\n† Stored-name hit is a nested Solution helper; helper identity check failed, not confirmed evaluator/tampering.',fontsize=8.2,va='bottom')
 fig.subplots_adjust(left=.12,right=.98,bottom=.15,top=.935,hspace=.25,wspace=.15)
 outputs=[]
 for ext in ['png','pdf']:
  path=D/f'completion_budget_projection_v2.{ext}';assert not path.exists();fig.savefig(path,dpi=240);outputs.append(dict(path=path.name,sha256=hashlib.sha256(path.read_bytes()).hexdigest(),size_bytes=path.stat().st_size))
 plt.close(fig)
 with(D/'FIGURE_BINDINGS_V2.json').open('x')as out:json.dump(dict(points=points,figures=outputs,analysis_sha256=hashlib.sha256((D/'analysis.json').read_bytes()).hexdigest(),estimators_recomputed=False),out,indent=2,sort_keys=True);out.write('\n')
